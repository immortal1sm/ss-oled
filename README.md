...
| **Weather** | 10s | Today: big °C temp, condition label, precipitation %, animated icon — spinning sun rays, falling rain, lightning flashes, snow, fog wisps, drifting clouds. Explicit "NO DATA" placeholder when the cache is empty or a fetch fails (we don\'t guess conditions we haven\'t retrieved) |
